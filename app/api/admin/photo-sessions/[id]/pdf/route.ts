...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // No chained .order() — that pattern has been observed dropping rows on this
  // table (sometimes all rows, sometimes only one category). Fetch unordered
  // and sort in JS, which is reliable and cheap given the row counts.
  ...
  console.log(
    `[pdf] session=${session.id} rows=${photos.length} ` +
